...
@section('content')
    <div class="container mx-auto max-w-5xl bg-white mt-4 border border-primary rounded-lg shadow-md overflow-y-auto h-[80vh]">
        ...
        <form action="{{ route('pets.store') }}" method="POST" onsubmit="this.querySelector('button[type=submit]').disabled = true;" enctype="multipart/form-data" class="px-8 pt-6 pb-8 space-y-6">
            ...
            @php
                $inputClasses = 'peer py-3 w-full placeholder-transparent rounded-md text-gray-700 ring-1 px-4 ring-gray-400 focus:ring-2 focus:ring-primary outline-none';
                $labelClasses = 'absolute left-3 -top-3 text-sm text-gray-600 bg-white px-1 transition-all peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-500 peer-placeholder-shown:top-3 peer-focus:-top-3 peer-focus:text-primary peer-focus:text-sm peer-focus:px-2';
                $textClass = 'peer py-3 w-full rounded-md text-gray-700 ring-1 px-4 ring-gray-400 focus:ring-2 focus:ring-primary outline-none';
            @endphp
            ...
            @foreach([
                ...
                <div class="relative">
                    <input type="{{ $name === 'age' ? 'number' : 'text' }}" name="{{ $name }}" value="{{ old($name) }}" class="{{ $inputClasses }}" placeholder=" " required>
            ...
            {{-- Description --}}
            <div class="relative">
                <textarea name="description" rows="4" class="{{ $textClass }}" placeholder="Write a short description about the pet...">{{ old('description') ?? '' }}</textarea>
            </div>
                ...
                <div class="flex space-x-4">
                    @foreach(['1' => 'Yes', '0' => 'No'] as $value => $text)
                        <label class="flex items-center space-x-2">
                            <input type="radio" name="spayed_neutered" value="{{ $value }}" {{ old('spayed_neutered') == $value ? 'checked' : '' }} required>
                            <span>{{ $text }}</span>
                        </label>
                    @endforeach
                </div>
