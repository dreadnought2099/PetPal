...
@section('content')
<div class="container mx-auto max-w-4xl bg-white mt-4 border border-primary rounded-lg shadow-md overflow-auto h-[80vh] space-y-10">
    <h1 class="flex gap-1 sticky top-0 py-3 px-4 text-xl font-bold bg-white z-10 justify-center">Pending Adoption <span class="text-primary">Requests</span></h1>
            ...
            @forelse($adoptions as $adoption)
                @if($adoption->status === 'pending')
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-3 border-b">{{ ucfirst($adoption->status) }}</td>
                        <td class="px-6 py-3 border-b">
                            <div class="flex gap-2">
                                <form action="{{ route('adopt.approve', $adoption) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="bg-green-500 text-white px-3 py-1.5 rounded hover:bg-green-700">Approve</button>
                                </form>

                                <form action="{{ route('adopt.reject', $adoption) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="bg-red-500 text-white px-3 py-1.5 rounded hover:bg-red-700">Reject</button>
                                </form>
                            </div>
                        </td>
                    </tr>
